...
clear
clc
cd 'F:\Kevin\Firm_Growth_Project\_Final\Sales\Data'

% location of code
addpath('F:\Kevin\Firm_Growth_Project\_Final\Sales\Code\Matlab')

% for using functions
addpath('F:\Kevin\Firm_Growth_Project\_Final\Sales\Code\Matlab\functions')

% for storing results
addpath('F:\Kevin\Firm_Growth_Project\_Final\Sales\Estimation\growth_simplified')
rng default;
...
tcap=100; %max choice is tcap=100;
...
% index is "a"
load('matlab\C_growth_ihs.mat');
load('matlab\t_vector_ihs.mat')
load('matlab\a_vector_ihs.mat')
load('matlab\W_boot_growth_ihs.mat')
...
L = @(x) lossfunction_growth_Luttmer(x,C_cap,t_vector_cap,a_vector_cap,W_choice_cap);
...
a = [-5,-5,-1,0,0,0]; % lower bound of randomization: Francesco choice
b = [5,5,5,5,5,5]; % upper bound of randomization

if lower_bound==0  
    lb = [-5,-5,-1,0,0,0]; %make lower bound at zero for variances AND autocorr
else
        lb = [-5,-5,-1,0,0,0];
end
...
% transform columns 1 and 2 back to alpha and delta
x_min(1:end,1) = exp( x_min(1:end,1) ) ./ (1 + exp( x_min(1:end,1) ) );
x_min(1:end,2) = exp( x_min(1:end,2) ) ./ (1 + exp( x_min(1:end,2) ) );
...
allresults_ordered=array2table(sortrows(allresults),'VariableNames',...
    {'L','alpha','delta','rho','sigma2_q','sigma2_e','lambda','exitflag'});

% minimand
x = table2array(allresults_ordered(1,2:7));
...
writetable(allresults_ordered(1:5,:), 'F:\Kevin\Firm_Growth_Project\_Final\Sales\Estimation\growth_simplified\Belgium_growth_Luttmer_estimates.csv')


%% 7) Show model fit

% Best estimates from simplified growth model
alpha_fit=table2array(allresults_ordered(1,2));
delta_fit=table2array(allresults_ordered(1,3));
rho_fit=table2array(allresults_ordered(1,4));
sigma2_q_fit=table2array(allresults_ordered(1,5));
sigma2_e_fit=table2array(allresults_ordered(1,6));
lambda_fit=table2array(allresults_ordered(1,7));

% Model implied moments
model_fit=covariance_growth_Luttmer(t_vector_cap,a_vector_cap,alpha_fit,delta_fit,rho_fit,sigma2_q_fit,sigma2_e_fit,lambda_fit);

figure
for tt=1:20
    plot([tt:21+tt],model_fit(t_vector_cap==tt),'r')
    hold on
    plot([tt:21+tt],C_cap(t_vector_cap==tt),'b')    
end
legend("Model","Data")
xlabel("Firm age")
ylabel("Autocovariance")
title("Model fit: Belgian Data, growth (Luttmer model)")
saveas(gcf,'F:\Kevin\Firm_Growth_Project\_Final\Sales\Estimation\growth_simplified\ModelFit_Belgium_growth_Luttmer.png')
