...
% 1) Loads data of firm growth rates in "new_vat_data_reshaped_growth.csv"
...
% 3) Computes vector C (correlation of growth rates across ages)

% 4) Computes weighting matrix W_boot with bootstrap

% CHANGES FROM SALES DATA:
    % 1. age 21 turned to age 14 due to data due to sample length
    % 2. age 22 turned to age 15
...
data=readmatrix('D:\Kevin\Firm_Growth_Project\_Final\Employment\Data\clean\reshaped\empl_new_vat_data_reshaped_growth_halt.csv');
data=data(:,2:end);
%% 2) Study missing values in the dataset and does some plots on firm demographics
...
%% 3) Computes vector C (autocovariance of growth rates across ages)

a_max=14; %maximum horizon for computation of autocovariance, limit imposed by sample length 1995-2017
% Remark: we could have picked a_max=15, but then N_{t,a=15} would have no values for some ages t (especially old ages)
...
for j=1:(T-a_max)
t_vector((j-1)*15+1:j*15)=j*ones(15,1);
a_vector((j-1)*15+1:j*15)=[0:14]';
end
...
save('matlab\C_growth_halt.mat','C')
save('matlab\t_vector.mat','t_vector')
save('matlab\a_vector.mat','a_vector')
...
save('matlab\W_boot_growth_halt.mat','W_boot')
